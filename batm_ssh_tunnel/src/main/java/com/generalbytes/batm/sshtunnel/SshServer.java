...
public class SshServer {
    ...
    public void start() throws IOException {
        ...
        sshd.setPort(port);
        // Since sshd 2.x the default host key algorithm changed from RSA to EC. Pin it to RSA/2048 so that
        // existing host_key files (generated by older versions) keep loading instead of being regenerated,
        // which would change the server fingerprint and break clients that pinned it in known_hosts.
        BouncyCastleGeneratorHostKeyProvider hostKeyProvider = new BouncyCastleGeneratorHostKeyProvider(new File(configDir, HOST_KEY_FILE).toPath());
