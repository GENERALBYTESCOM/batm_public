...
public class ZcoinRPCWallet implements IWallet{
    private static final Logger log = LoggerFactory.getLogger(ZcoinRPCWallet.class);
    private static final String CRYPTO_CURRENCY = CryptoCurrency.XZC.getCode();

    public ZcoinRPCWallet(String rpcURL, String accountName) {
        this.rpcURL = rpcURL;
    ...
    public String sendCoins(String destinationAddress, BigDecimal amount, String cryptoCurrency, String description) {
        if (!CRYPTO_CURRENCY.equalsIgnoreCase(cryptoCurrency)) {
            log.error("Zcoind wallet error: unknown cryptocurrency.");
            return null;
        }
    ...
    public String getCryptoAddress(String cryptoCurrency) {
        ...
        try {
            List<String> addressByAccount = getClient(rpcURL).getAddressesByAccount(accountName);
            if (addressByAccount == null || addressByAccount.size() == 0) {
                return null;
            }else {
