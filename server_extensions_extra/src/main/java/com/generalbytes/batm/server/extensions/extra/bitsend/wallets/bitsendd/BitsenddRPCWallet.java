...
public class BitsenddRPCWallet implements IWallet{
    ...
    private static final String CRYPTO_CURRENCY = CryptoCurrency.BSD.getCode();
    ...
    public String sendCoins(String destinationAddress, BigDecimal amount, String cryptoCurrency, String description) {
        if (!CRYPTO_CURRENCY.equalsIgnoreCase(cryptoCurrency)) {
            log.error("Bitsend wallet error: unknown cryptocurrency.");
            return null;
        }

        log.info("Bitsend sending coins from " + accountName + " to: " + destinationAddress + " " + amount);
        try {
    ...
    public BigDecimal getCryptoBalance(String cryptoCurrency) {
        if (!CRYPTO_CURRENCY.equalsIgnoreCase(cryptoCurrency)) {
            log.error("Bitsend wallet error: unknown cryptocurrency: " + cryptoCurrency);
            return null;
        }
        try {
