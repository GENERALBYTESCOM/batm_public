...
public class DecentdRPCWallet implements IWallet {
    ...
    public String sendCoins(String destinationAddress, BigDecimal amount, String cryptoCurrency, String description) {
        if (!CRYPTO_CURRENCY.equalsIgnoreCase(cryptoCurrency)) {
            log.error("Decentd wallet error: unknown cryptocurrency.");
            return null;
        }

        log.info("Decentd sending coins from " + accountName + " to: " + destinationAddress + " " + amount);
        try {
    ...
    public BigDecimal getCryptoBalance(String cryptoCurrency) {
        if (!CRYPTO_CURRENCY.equalsIgnoreCase(cryptoCurrency)) {
            log.error("Decentd wallet error: unknown cryptocurrency: " + cryptoCurrency);
            return null;
        }
        try {
