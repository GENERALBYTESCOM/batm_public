...
public class aquaDigest implements ParamsDigest {
    ...
    public aquaDigest(String apiSecret) throws GeneralSecurityException {
    ...
    protected byte[] getMacData(RestInvocation restInvocation) {
        try {
            return new ObjectMapper().writer().writeValueAsBytes(aquaMacData.from(restInvocation));
