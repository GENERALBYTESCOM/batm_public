...
public abstract class AbstractAternityRPCPaymentSupport implements IPaymentSupport{
    ...
    public abstract BigDecimal calculateTxFee(String senderAddress, String destinationAddress, BigDecimal amount, RPCClient client);
        ...
        if (watcher == null) {
            try {
                if (client.getNetworkInfo().connections() > 0) {
                    watcher = new AeternityRPCBlockchainWatcher(client);
    ...
    private TXForBroadcast createTransaction(BitcoindRpcClient.RawTransaction sourceTransaction, PaymentRequest request, IPaymentRequestSpecification spec, BigDecimal remain) {
        ...
        try {
            ...
            IPaymentOutput paymentOutput = fwdOutputs.get(0);
