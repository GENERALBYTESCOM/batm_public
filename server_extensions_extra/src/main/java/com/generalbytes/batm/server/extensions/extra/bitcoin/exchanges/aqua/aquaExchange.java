...
import com.generalbytes.batm.server.extensions.extra.bitcoin.exchanges.aqua.dto.OrderSide;
import com.generalbytes.batm.server.extensions.extra.bitcoin.exchanges.aqua.dto.OrderType;
...
public class aquaExchange implements IExchangeAdvanced, IRateSourceAdvanced {
    ...
    private final IaquaAPI api;

    public aquaExchange(String apiKey, String apiSecret, String preferredFiatCurrency) throws GeneralSecurityException {
        this.api = IaquaAPI.create(apiKey, apiSecret);
    ...
    private BigDecimal calculatePrice(String cryptoCurrency, String fiatCurrency, BigDecimal cryptoAmount, OrderSide orderSide) {
        ...
        return call("calculate " + orderSide + " price", () -> api.quoteOrder(new QuoteRequest(cryptoAmount, getMarketSymbol(cryptoCurrency, fiatCurrency), orderSide)).fillPrice.multiply(cryptoAmount));
    }
    // private BigDecimal calculatePrice(String cryptoCurrency, String fiatCurrency, BigDecimal cryptoAmount, OrderSide orderSide) {
    //     if (!isCryptoCurrencySupported(cryptoCurrency) || !isFiatCurrencySupported(fiatCurrency)) {
    //         return null;
    ...
    class OrderTask implements ITask {
        ...
        public boolean onCreate() {
            ...
            orderId = call("task submitLimitOrder", () -> {
                QuoteRequest quoteRequest = new QuoteRequest(cryptoAmount, getMarketSymbol(cryptoCurrency, fiatCurrencyToUse), orderSide);
                QuoteResponse quote = api.quoteOrder(quoteRequest);

                OrderRequest orderRequest = new OrderRequest(cryptoAmount, getMarketSymbol(cryptoCurrency, fiatCurrencyToUse), orderSide);
    ...
    public BigDecimal getExchangeRateLast(String cryptoCurrency, String fiatCurrency) {
        ...
        try {
            ...
            RateResponse response = mapper.readValue(json, RateResponse.class);
